...
//todo Formun submit butonuna basıldığında
ekleBtn.addEventListener("click", (e) => {
  e.preventDefault(); //todo Reloadı engeller
  gelirler = gelirler += Number(gelirInput.value); //? String Eklemeyi Engeller

  //todo gelirlerin kalıcı hale gelmesi için localStorage'a kopyalıyoruz
  localStorage.setItem("gelirler", gelirler);

  //todo Input değerini sıfırlar
  ekleFormu.reset();

  //todo Değişikleikleri sonuc tablosuna yazan fonksiyon
  hesaplaVeGuncelle();
});
//todo Sayfa her yüklendikten sonra çalışan event
window.addEventListener("load", (e) => {
  gelirler = Number(localStorage.getItem("gelirler"));

  //todo local storageden harcama listesini okuyarak global dizimize saklıyoruz
  harcamaListesi = JSON.parse(localStorage.getItem("harcamalar")) || [];

  harcamaListesi.forEach((harcama) => harcamayiDomaYaz(harcama));

  //todo şuan ki tarihi date olarak tarih inputuna bas

  tarihInput.valueAsDate = new Date();
  hesaplaVeGuncelle();
});

//todo harcama formu submit edildiğinde çalışır
harcamaFormu.addEventListener("submit", (e) => {
  e.preventDefault();

  //todo  yeni harcama bilgileri ile bir obje oluştur
  const yeniHarcama = {
    id: new Date().getTime(), //todo sistem saatini (ms olarak) verir.
    tarih: tarihInput.value,
    alan: harcamaAlaniInput.value,
    miktar: miktarInput.value,
  };

  //todo yeni harcama objesini diziye ekle
  harcamaListesi.push(yeniHarcama);

  //todo dizimizin son halini kalıcı hale getirmek  için kalıcı hale getiriyoruz
...
const harcamayiDomaYaz = ({ id, tarih, alan, miktar }) => {
  ...
  <tr>
   ...
   <td>${miktar} </td>
   <td>
    <i  id=${id} class="fa-solid fa-trash-can text-warning" type="button"></i>
   </td>
</tr>
  `;
};
...
harcamaBody.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("fa-trash-can")) {
    //todo Dom'dan ilgili rowu silme işlemini yaptık
    e.target.parentElement.parentElement.remove();
    const id = e.target.id;
    console.log(id);

    //todo Dizideki ilgi objeyi sildik
    harcamaListesi = harcamaListesi.filter((harcama) => harcama.id != id);

    //todo Silinmiş Yeni Diziyi localStorage'a yansıt
    localStorage.setItem("harcamalar", JSON.stringify(harcamaListesi));

    //todo her satır silindikten sonra yeni değerleri hesapla ve doma yaz
    hesaplaVeGuncelle();
  }
});

//todo Temizle butonun basıldığı  zaman çalış
temizleBtn.addEventListener("click", () => {
  if (confirm("Silmek istediğinize emin misiniz?")) {
    harcamaListesi = []; //todo Ram'deki harcama listesini sil
    gelirler = 0; //todo Ram'deki  gelirleri sil
    localStorage.clear(); //todo localStorage'deki tüm verileri sil
    harcamaBody.innerHTML = ""; //todo Dom'daki tüm  harcamaları sil
    hesaplaVeGuncelle(); //todo Sonuç tablsounda ki (DOM) gelirler, giderler ve kalan değerleri sil
  }
});

//

//* böyle tek tek inputları yakalyıp change venti tanımlamak yerine aşağıda yakladığımız formun childNodeslarında gezinip her birine change eventı tanımlamış olduk.
...
// //! formdaki bilgileri toplamak için bir obje tanımladık. Ve key isimleriyle inputlardaki name attirubutelerindeki isimler ile aynı tanımladık.
// let initialState = {
//   id: "",
//   yer: "",
//   tarih: "",
